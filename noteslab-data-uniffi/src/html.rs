...
use noteslab_data::{ModelType, NoteslabData};

pub fn parse_markdown_to_preview_html(data: &NoteslabData, text: String) -> String {
...
where
    ...
    W: StrWrite,
{
    fn new(iter: I, writer: W, data: &'a NoteslabData) -> Self {
        Self {
            iter,
            writer,
            end_newline: true,
    ...
    fn start_tag(&mut self, tag: Tag<'a>) -> io::Result<()> {
        match tag {
                ...
                if dest.len() == 34 && dest.starts_with(":/") {
                    ...
                    let scheme = self
                        ...
                        .map(|t| match t.item_type {
                            ModelType::Note => "hgwebs1s",
                            _ => "noteslab-files",
                        })
                        .unwrap_or("noteslab-corrupt-files");
            ...
            Tag::Image(_link_type, dest, title) => {
                self.write("<img src=\"")?;
                if dest.len() == 34 && dest.starts_with(":/") {
                    escape_href(&mut self.writer, &format!("noteslab-files:///{}", &dest[2..]))?;
                } else {
                    escape_href(&mut self.writer, &dest)?;
                }
                self.write("\" alt=\"")?;
...
pub fn push_html<'a, I>(s: &mut String, iter: I, data: &'a NoteslabData)
where
    I: Iterator<Item = Event<'a>>,
{
