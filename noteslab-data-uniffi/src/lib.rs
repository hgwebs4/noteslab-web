...
use noteslab_data::{
    ...
    DatabaseError, Folder, Note, Resource, NoteslabData, SearchBodyOption, UpdateSource,
};
use std::path::Path;
...
uniffi::include_scaffolding!("noteslab");
...
pub struct NoteslabAndroidData {
    data: NoteslabData,
    rt: Runtime,
    _log_handle: log4rs::Handle,
}

impl NoteslabAndroidData {
    pub fn new(
        data_dir: String,
        resource_dir: String,
        ...
        let data = NoteslabData::new(Path::new(&data_dir), Path::new(&resource_dir))?;
        Ok(Self {
            data,
            rt,
    ...
    pub fn replace_folder(&self, folder: FFIFolder) -> Result<(), DatabaseError> {
        self.data
            .db
            .replace_folder(&folder.into(), noteslab_data::UpdateSource::LocalEdit)
    }
